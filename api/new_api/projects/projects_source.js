...
class ProjectsFileSystem {
    ...
    async readProjectFolder(data, callback) {
        ...
        async function walkDir(dir, result = {}) {
            ...
            for(let item of list) {
                if (item === 'node_modules' || item === 'package-lock.json' ){}
                else {
    ...
    async createFileInProject(data, callback){
        let userId = data.user_id;
        let projectId = data.project_id;
        let fileName = data.file_name;

    }
    async createFolderInProject(data, callback){
        let userId = data.user_id;
        let projectId = data.project_id;
        let folderName = data.folder_name;

    }
    async saveProjectFiles (data,callback){
        let projectName = data.project_name;
        let projectId = data.project_id;
        let userId = data.user_id;
        let filesObject = data.files;
